...
from spotipy import Spotify, SpotifyOAuth, SpotifyOauthError, CacheFileHandler
...
class Login:
    ...
    def __init__(self, client_id=None, client_secret=None, redirect_uri=None):
        ...
        if client_id is None or client_secret is None or redirect_uri is None:
            client_id = self._settings.get_setting('client_id')
            client_secret = self._settings.get_setting('client_secret')
            redirect_uri = self._settings.get_setting('redirect_uri')
        ...
        try:
            self._auth = SpotifyOAuth(scope=Login._scope,
                                      ...
                                      cache_handler=CacheFileHandler(cache_path=self._settings.get_path() + '\\.cache')
                                      )
        except Exception:
